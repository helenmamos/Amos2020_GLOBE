...
#  
# PURPOSE
# Set up the Observation class and class properties associated with GLOBE data. 
#
# RESOURCES
...
# XX Jul 2019 - MJ Starke - v1.0 written
# 17 Oct 2019 - HM Amos   - code adapted for Amos & Starke et al. (2020)
#
# CITATION
...
class Observation:
    ...
    @property
    def mosquito_genus(self) -> List[str]:
        """
        :return:  Mosquito reported in this observation
        
        Added by HM Amos, 20 Dec 2019
        """
        mosquito_genus = ["Genus"]
        return [key for key in mosquito_genus if self.soft_get(key) in ["Culex","Aedes","Anopheles"]]

    @property
    def which_muc(self) -> Optional[str]:
        """
        :return: Returns the MUC classification assigned to this observation, or None if 
        MUC was computed
        
        Added by HM Amos, 20 Dec 2019
        """
        return self.soft_get("MucCode")
...
# There are a number of situations where it is physically meaningful to report
# two types of obscurations (e.g., smoke & haze), so remove this as a flag.
# An observation starts to become suspicious when three or more obscurations
# are reported and suggests the participant is confused and just checking boxes.
# (hma, 2020-01-21)                    
...
# Remove 00 flag from GLOBE Observer paper because it's not possible to trip
# the 00 flag based on how the app is set up. 
# (hma, 2020-01-21)
...
# Remove HO and HC flags from GLOBE Observer paper. These flags aren't meaningful
# for the app because they're just a consequence of how the app is set up and
# don't mean the participant did anything wrong. The flags are meaningful for
# traditional GLOBE data. (hma, 2020-01-22)            
...
#                self.flag("HC")
    ...
    def photo_urls(self) -> Dict[str, str]:
        """
        :return: Gets a dictionary of direction=url pairs for each direction 
        that has a photo for this observation. A key will be absent if no photo 
        exists.  The value of a given key is not guaranteed to be a valid url; 
        in particular, the value may be "rejected" if a photo was submitted but
        was rejected from the GLOBE database.
        
        MODIFICATION HISTORY
        04 Dec 2019 - HM Amos - added keywords for tree and mosquito observations
        ...
        for th in ["Tree"]:
            try:
                ret[th] = self["{}PhotoUrls".format(th)]
            except KeyError:
                pass  
        for mhm in ["WaterSource","LarvaFullBody"]:
            try:
                ret[mhm] = self["{}PhotoUrls".format(mhm)]
            except KeyError:
                pass    
        return ret
